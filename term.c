#include <ctype.h>
#include <signal.h>
#include <string.h>
#include <sys/ioctl.h>
#include <stdio.h>
#include <stdlib.h>
#include <termios.h>
#include <unistd.h>
#include <stdbool.h>

#include "term.h"

#define LINE_N 3
#define LINE_OFFSET 1

#define ANSI_FG_GREEN "\033[32m"
#define ANSI_FG_RED "\033[31m"
#define ANSI_RESET "\033[0m"
#define ANSI_CLEAR "\033[2J"
#define ANSI_MOVE_LEFT(C) "\033[" #C "D"
#define ANSI_MOVE_RC(R, C) "\033[" #R ";" #C "H"

#define SL_HEIGHT 1

typedef char color;
#define COLOR_RESET 0
#define COLOR_ACC 1
#define COLOR_WRNG 2

typedef struct Term {
  int width;
} Term;
Term t;

typedef struct _Reference {
  Reference base;
  // char s[TYPETEST_BUF_SIZE];
  color acc[TYPETEST_BUF_SIZE];
  int roww[128];
  int rowc;
} _Reference;
_Reference ref;
Reference *term_ref = (Reference *) &ref;

reference_generator ref_gen;
bool stop_gen = false;

typedef struct Cursor {
  int posn;
  int posr;
  int posc;
} Cursor;
Cursor cur;

struct termios orig_term;

char sl[STATUS_LINE_SIZE];


static void cur_pos_inc();
static void cur_pos_dec();
static void cur_replace(char, color);
static void jump_to_cur();
static int get_cur_offset();

static void term_wrap();
static void term_gen_ref_line();
static void term_write_str();
static char *get_color_escape(color);
static void update_term_width(int signo);

static void enable_raw_mode();


void term_init(reference_generator gen) {
  ref_gen = gen;

  cur.posc = 0;
  cur.posr = 0;
  cur.posn = 0;

  ref.base.len = 0;

  enable_raw_mode();

  update_term_width(SIGWINCH);
  signal(SIGWINCH, update_term_width);

  for (int i = 0; i < LINE_N; i++) {
    term_gen_ref_line();
  }
  term_wrap();

  term_write_str();
}

int term_send_char(char ch, bool is_acc) {
  if (is_acc) {
    cur_replace(ch, COLOR_ACC);
    ref.acc[cur.posn] = COLOR_ACC;
  } else {
    cur_replace(ch, COLOR_WRNG);
    ref.acc[cur.posn] = COLOR_WRNG;
  }

  cur_pos_inc();
  jump_to_cur();

  return cur.posn;
}

int term_send_backspace(char replace) {
  cur_pos_dec();
  cur_replace(replace, COLOR_RESET);

  return cur.posn;
}

void term_update_status_line(char *new_sl) {
  char buf[STATUS_LINE_SIZE + 32];
  int len = snprintf(
    buf,
    sizeof(buf),
    "\033[H\033[0K\033[0m%s",
    new_sl
  );

  write(STDOUT_FILENO, buf, len);

  jump_to_cur();

  strcpy(sl, new_sl);
}

void disable_raw_mode() {
  tcsetattr(STDIN_FILENO, TCSAFLUSH, &orig_term);
}

static void enable_raw_mode() {
  tcgetattr(STDIN_FILENO, &orig_term);
  atexit(disable_raw_mode);

  struct termios raw = orig_term;
  raw.c_lflag &= ~(ICANON | ECHO);

  tcsetattr(STDIN_FILENO, TCSAFLUSH, &raw);
}


static void cur_pos_inc() {
  cur.posn++;
  cur.posc++;
  if (cur.posc >= ref.roww[cur.posr]) {
    cur.posc = 0;
    cur.posr++;

    int genc = cur.posr + LINE_OFFSET - ref.rowc + 2;
    for (int i = 0; i < genc; i++) {
      term_gen_ref_line();
    }
    term_wrap();
    term_write_str();
  }
}

static void cur_pos_dec() {
  if (cur.posn == 0) return;

  cur.posn--;

  if (cur.posc > 0) {
    cur.posc--;
  } else {
    cur.posr--;
    cur.posc = ref.roww[cur.posr] - 1;

    term_write_str();
  }
}

static void cur_replace(char replace, color acc) {
  char buf[128];

  int offset = get_cur_offset();
  int len = snprintf(
    buf,
    sizeof(buf),
    "%s\033[%d;%dH%c\033[%d;%dH",
    get_color_escape(acc), SL_HEIGHT + cur.posr + 1 - offset, cur.posc + 1, replace, SL_HEIGHT + cur.posr + 1 - offset, cur.posc + 1
  );
  write(STDOUT_FILENO, buf, len);
}

static void jump_to_cur() {
  char buf[128];

  int offset = get_cur_offset();
  int len = snprintf(
    buf,
    sizeof(buf),
    "\033[%d;%dH",
    SL_HEIGHT + cur.posr + 1 - offset, cur.posc + 1
  );
  write(STDOUT_FILENO, buf, len);
}

static int get_cur_offset() {
  int offset = (cur.posr + 1) - (LINE_N - LINE_OFFSET);
  if (offset < 0) offset = 0;

  return offset;
}

static char *get_color_escape(color c) {
  switch (c) {
  case COLOR_RESET:
    return ANSI_RESET;
  case COLOR_ACC:
    return ANSI_FG_GREEN;
  case COLOR_WRNG:
    return ANSI_FG_RED;
  }

  return "";
}

static void term_wrap() {
  int prev_space_pos = 0;
  int put = 0;

  ref.rowc = 1;
  for (int i = 0; i < TYPETEST_BUF_SIZE; i++) {
    if (isspace(ref.base.s[i])) {
      ref.base.s[i] = ' ';

      if (i - put > t.width) {
        ref.base.s[prev_space_pos] = '\n';
        ref.roww[ref.rowc - 1] = prev_space_pos - put + 1;
        ref.rowc++;
        put = prev_space_pos + 1;
      }

      prev_space_pos = i;
    } else if (ref.base.s[i] == '\0') {
      if (i - put > t.width) {
        ref.base.s[prev_space_pos] = '\n';
        ref.roww[ref.rowc - 1] = prev_space_pos - put + 1;
        ref.rowc++;
        put = prev_space_pos + 1;
      }
      ref.roww[ref.rowc - 1] = i - put + 1;
      break;
    }
  }
}

static void term_gen_ref_line() {
  if (stop_gen) {
    return;
  }

  char buf[64];

  int rem = t.width - 1;
  while (rem > 0) {
    int len = ref_gen(buf);
    if (len == -1) {
      stop_gen = true;
      break;
    }

    memcpy(ref.base.s + ref.base.len, buf, len);
    ref.base.len += len;
    ref.base.s[ref.base.len++] = ' ';

    rem -= len + 1;
  }
  ref.base.s[ref.base.len] = '\0';
}

static void term_write_str() {
  char buf[TYPETEST_BUF_SIZE * 8];
  char *p = buf;
  size_t rem = sizeof(buf);
  size_t len = strlen(ref.base.s);

  int n = snprintf(
    buf,
    sizeof(buf),
    ANSI_CLEAR "\033[%d;0H",
    SL_HEIGHT + 1
  );

  if (n < 0) return;
  p += n; rem -= n;

  int offset = get_cur_offset();
  int start = 0;
  for (int i = 0; i < offset; i++) {
    start += ref.roww[i];
  }

  int end = start;
  for (int i = 0; i < LINE_N; i++) {
    end += ref.roww[i + offset];
  }

  for (int i = start; i < cur.posn; i++) {
    char *esc = get_color_escape(ref.acc[i]);
    char n = strlen(esc);
    memcpy(p, esc, n);
    p += n;
    *p++ = ref.base.s[i];
    rem -= n + 1;
  }

  n = snprintf(p, rem, ANSI_RESET);
  if (n < 0) return;
  p += n; rem -= n;

  for (int i = cur.posn; i < end; i++) {
    *p++ = ref.base.s[i];
    rem--;
  }

  n = snprintf(p, rem, "\033[%d;0H", SL_HEIGHT + 1);
  if (n < 0) return;
  p += n; rem -= n;

  write(STDOUT_FILENO, buf, p - buf);
}

static void update_term_width(int signo) {
  struct winsize wins;
  ioctl(STDOUT_FILENO, TIOCGWINSZ, &wins);

  t.width = wins.ws_col - 1;

  term_wrap();
  term_write_str();

  int c = 0, pc = 0;
  for (int i = 0; i < ref.rowc; i++) {
    c += ref.roww[i];
    if (c >= cur.posn) {
      cur.posr = i;
      cur.posc = cur.posn - pc;
      break;
    }
    pc = c;
  }

  char buf[10];
  int len = snprintf(
    buf,
    sizeof(buf),
    "\033[%d;%dH",
    SL_HEIGHT + cur.posr + 1, cur.posc + 1
  );
  write(STDOUT_FILENO, buf, len);
}
